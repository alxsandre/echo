...
export function PlayButton({
  ...
  const visual = playing ? (
    <span className="flex gap-0.5">
      <span className={`${barClass} animate-pulse-bar bg-bg rounded-sm`} />
      <span className={`${barClass} animate-pulse-bar-delayed bg-bg rounded-sm`} />
    </span>
  ) : (
    <svg
      width="10"
      height="11"
      viewBox="0 0 10 11"
      fill={isAccent ? "var(--accent)" : "var(--text-muted)"}
    >
